...
EOF
	...
	source terraform-gen/function/variable.sh
  pod_info
...
EOF
    ...
    if [[ ${!VM_OS} = $(echo ${!VM_OS} | egrep -i -h "centos|redhat|rhel|rocky|alma") ]]; then
...
EOF
    ...
    if [[ ${!VM_OS} = $(echo ${!VM_OS} | egrep -i -h "ubuntu|trusty|xenial|bionic|focal|jammy|proxmox") ]]; then
      if grep -q LAB ${ENV_FILE}; then
        cat <<EOF >${TMP_DIR}/cloudinit${i}.cfg
				#cloud-config
				hostname: ${!VM_NAME}
